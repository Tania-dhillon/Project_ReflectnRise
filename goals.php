...
// unit and due date. Completed goals are shown in their own
// section similar to the provided design.
// ------------------------------------------------------------
...
$activeGoals = array_values(array_filter($allGoals, fn($g) => $g['status'] !== 'Completed'));
$completedGoals = array_values(array_filter($allGoals, fn($g) => $g['status'] === 'Completed'));
?>
                        ...
                        <div class="goal-card-left">
                            ...
                            <div>
                                <div class="goal-title-row">
                                    <h3><?= e($goal['title']); ?></h3>
                                    <?php if ($goal['status'] === 'Completed'): ?><span class="goal-complete-check"><i class="bi bi-check-circle"></i></span><?php endif; ?>
                                </div>
                                <div class="goal-meta-row">
                                    <span class="goal-category-pill"><?= e($icon . ' ' . $goal['category']); ?></span>
                                    <?php if (!empty($goal['due_date'])): ?><span>Due <?= date('M j, Y', strtotime($goal['due_date'])); ?></span><?php endif; ?>
                                </div>
                            </div>
                        </div>

                        <div class="goal-card-actions">
                            <button type="button" class="goal-action-btn" data-bs-toggle="modal" data-bs-target="#<?= e($goalModalId); ?>"><i class="bi bi-pencil"></i></button>
                                ...
                                <div class="goals-grid-two mb-3">
                                    <select name="category" class="form-select goals-input">
                                        <?php foreach ($goalCategories as $cat => $emoji): ?>
                                            <option value="<?= e($cat); ?>" <?= $goal['category'] === $cat ? 'selected' : ''; ?>><?= e($emoji . ' ' . $cat); ?></option>
                                        <?php endforeach; ?>
                                    </select>

                                    <select name="status" class="form-select goals-input">
                                        <?php foreach ($goalStatuses as $status): ?>
                                            <option value="<?= e($status); ?>" <?= $goal['status'] === $status ? 'selected' : ''; ?>><?= e($status); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
